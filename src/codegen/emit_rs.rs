    ...
    match schema.get("type").and_then(|v| v.as_str()) {
        ...
        Some("object") => {
            if schema.get("properties").is_some() {
                // Nested object with known properties — would need a named struct.
...
mod tests {
    ...
    #[test]
    fn test_json_schema_to_rust_type_null() {
        let schema = serde_json::json!({"type": "null"});
        assert_eq!(json_schema_to_rust_type(&schema), "()");
    }

    #[test]
    fn test_emit_rust_types_empty_tools() {
        let output = emit_rust_types(&[], "empty-server");
        // Should still produce valid header and client struct
        assert!(output.contains("Auto-generated by mcplug"));
        assert!(output.contains("pub struct EmptyServerClient"));
        // Should not contain any Args struct
        assert!(!output.contains("Args {"));
    }

    #[test]
    fn test_sanitize_identifier_special_chars_in_description() {
        // Special chars in identifiers are replaced with underscores
        assert_eq!(sanitize_identifier("hello@world#123"), "hello_world_123");
